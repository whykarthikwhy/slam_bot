...
def generate_launch_description():
    ...
    spawn_robot = Node(
        ...
        arguments=[
            ...
            '-x', '-2.0', # Start a bit further back to give time for rviz to load and connect
            '-y', '-2.0',
        ],
        output='screen'
    )  
    ...
    slam_params_path = os.path.join(get_package_share_directory('slam_bot'), 'config', 'slam_params_localisation.yaml')

    slam_toolbox_loc = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
