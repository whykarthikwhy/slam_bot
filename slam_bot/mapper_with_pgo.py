...
class Mapper_PGO(Node):
    def __init__(self):
        super().__init__('mapper_with_pgo')
        self.odom_tf_subscriber = self.create_subscription(TFMessage, "/tf" , self.odom_tf_callback, 10)
        self.lidar_subscriber = self.create_subscription(LaserScan, "/scan", self.lidar_callback, 3)
        self.map_publisher = self.create_publisher(OccupancyGrid, "/map", 10)
        self.pose_array_publisher = self.create_publisher(PoseArray, "/pose_array", 10)
        self.keyframe_pose_array = PoseArray()
        self.keyframe_pose_array.header.frame_id = 'map'
        self.tf_publisher = self.create_publisher(TFMessage, "/pgo/tf", 10)
        ...
        self.keyframe_min_dist = 0.3
        self.keyframe_min_angle = 0.3
        self.loop_closure_min_nodes = 50
        self.loop_closure_min_dist = 0.4
        self.max_icp_iteration = 20
        self.icp_cooloff = 15
        self.icp_cooloff_max = 15
        self.tot_d = 0.0
        self.tot_r = 0.0
        self.count = 0
        ...
        for t in msg.transforms:
            if t.header.frame_id == 'odom' and t.child_frame_id == 'base_link':
                if self.is_first_tf:
                    ...
                    self.get_logger().info("First transform published")
    ...
    def get_map_pose(self, time_stamp):
        if not self.tf_buffer.can_transform('map', 'base_link', time_stamp, timeout=Duration(seconds=0.1)):
            #self.get_logger().warn('map->base_link not available for this stamp yet')
            return None, None, None
        try:
            map_to_base = self.tf_buffer.lookup_transform(
                'map', 'base_link', time_stamp, timeout=Duration(seconds=0.1)
            )
    ...
    def update_grid_once(self, robot_x, robot_y, yaw, scan_cart):
    ...
    def remap(self):
        ...
        #self.get_logger().info(f"Remapping. Start Time: {t0}")
        self.log_odds = [0.0] * (self.width * self.height)
        for node in self.graph.pose_nodes:
            pose = node.get_pose()
            id = node.get_id()
            scan = self.graph.scans[id]
            self.update_grid_once(pose[0,0], pose[1,0], pose[2,0], scan)
          
        self.map_publisher.publish(self.grid)
        dt = (time.time() - t0) - self.start_time
        self.get_logger().info(f"Remap took {dt} ns over {self.graph.node_count} nodes")

    def update_pose_array(self, curpose):    
        mat = tft.translation_matrix((curpose[0,0], curpose[1,0], 0.0)) @ tft.euler_matrix(0.0, 0.0, curpose[2,0])
        ...
        self.keyframe_pose_array.poses.append(pose)
        self.pose_array_publisher.publish(self.keyframe_pose_array)

    def stamp_to_ns(self, stamp):
        return stamp.sec * 10**9 + stamp.nanosec

    def mapper(self):
        #self.get_logger().info(f"Mapper called at {time.time() - self.start_time}")
        cur_scan = self.scan
        time_stamp = cur_scan.header.stamp
        if self.stamp_to_ns(time_stamp) > self.stamp_to_ns(self.map_to_odom.header.stamp):
            return
        ...
        pose_result = self.get_map_pose(time_stamp)
        ...
        if self.is_first_scan:
            ...
            self.update_pose_array(cur_pose)
            self.update_grid_once(cur_pose[0,0], cur_pose[1,0], cur_pose[2,0], cur_scan_cart)
        ...
        if self.graph.node_count > self.loop_closure_min_nodes:
            ...
            if dist_min < self.loop_closure_min_dist:
                ...
                self.publish_map_to_odom_tf(final_pose_odom, final_pose_map)  
                self.remap()
    ...
    def publish_map_to_odom_tf(self, final_pose_odom, final_pose_map):
...
def main(args = None):
    ...
    mapper_with_pgo = Mapper_PGO()
    rclpy.spin(mapper_with_pgo)
    mapper_with_pgo.destroy_node()
    rclpy.shutdown()
